...
build-dev reth_path="":
    ...
    if [ -z "{{reth_path}}" ]; then
        if [ -f .cargo/config.toml ]; then
            ...
            echo "⚠️  First time setup needed: just build-dev /absolute/reth"
            exit 1
        fi
    else
...
check-dev-template:
    ...
    [ -z "$M" ] && echo "Check reth dev template passed" || (echo "❌ Check reth dev template failed: Missing: $M" && exit 1)
...
build-docker-dev reth_path="":
    ...
    if [ -z "{{reth_path}}" ]; then
        if [ -d .cargo/reth ]; then
            ...
            echo "⚠️  First time setup needed: just build-docker-dev /absolute/reth"
            exit 1
        fi
    else
